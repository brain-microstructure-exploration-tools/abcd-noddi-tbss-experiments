...
parser = argparse.ArgumentParser(description='concatenate Phillips dMRI scans from extracted ABCD Study images')
...
for subdir in extracted_images_path.iterdir():
    ...
    if len(dwi_dir_list) != 1:
        print(f"Encountered directory {subdir} in the extracted images path with unexpected contents. Skipping this one.")
    ...
    dwi_dirs_for_this_scan = list(extracted_images_path.glob(f'*/{subject_level_dirname}/{timepoint_level_dirname}/dwi'))
    row_lookup = df[df.filename == f'{subdir.stem}.tgz']

    if len(row_lookup)<1 :
        raise Exception(f"Could not find a row in {table_path} for filename {subdir.stem}.tgz, even though there is an extracted directory {subdir}.")
    if len(row_lookup)>1:
        raise Exception(f"Table {table_path} has multiple rows for filename {subdir.stem}.tgz.")
    ...
    if not is_phillips:
        raise Exception(f"Weird. Found multiple directories in {extracted_images_path} containing subdirectories {subject_level_dirname}/{timepoint_level_dirname}, but this is not supposed to be a Phillips scan so this is unexpected.")
    ...
    if len(dwi_dirs_for_this_scan) != 2:
        raise Exception(f"Expected there to be exactly two parts to this Phillips scan.")

    dwi_dir1, dwi_dir2 = dwi_dirs_for_this_scan
    bval1, bvec1 = read_bvals_bvecs(str(get_unique_file_with_extension(dwi_dir1, 'bval')), str(get_unique_file_with_extension(dwi_dir1, 'bvec')))
    bval2, bvec2 = read_bvals_bvecs(str(get_unique_file_with_extension(dwi_dir2, 'bval')), str(get_unique_file_with_extension(dwi_dir2, 'bvec')))
    ...
    dwi_dir_cat = extracted_images_path/archive_name/(dwi_dir1.parent.parent.name)/(dwi_dir1.parent.name)/(dwi_dir1.name)
    ...
    if row1.site_id_l != row2.site_id_l:
        raise Exception(f"Site ID in the table {table_path} differs between {archive_name1} and {archive_name2}. These should have been two parts of the same scanning session, so somthing is wrong.")
    if row1.mri_info_manufacturer != row2.mri_info_manufacturer:
        raise Exception(f"MRI manufacturer in the table {table_path} differs between {archive_name1} and {archive_name2}. These should have been two parts of the same scanning session, so somthing is wrong.")
    row_cat = pd.Series(
        {
    ...
    df_after_cat.drop(index=[f'{archive_name1}.tgz', f'{archive_name2}.tgz'], inplace=True)
    ...
    try:
        ...
        everything_exists = dwi_dir_cat.exists() and bval_cat_path.exists() and bvec_cat_path.exists() and img_cat_path.exists() and json1_dst_path.exists() and json2_dst_path.exists()
        ...
        error_msg = f"Error encountered while writing concatenated files to {dwi_dir_cat}."
        if dwi_dir_parent1.exists() and dwi_dir_parent2.exists():
            if dwi_dir_parent_cat.exists():
                shutil.rmtree(dwi_dir_parent_cat)
                df_before_cat.to_csv(table_path, index=False) # restore table in case it was updated
