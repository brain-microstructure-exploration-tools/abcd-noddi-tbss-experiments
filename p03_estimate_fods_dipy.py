...
from common import (
    get_unique_file_with_extension,
    write_dipy_response,
    read_dipy_response,
...
parser = argparse.ArgumentParser(
    description="estimates fiber orientation distribution images using constrained spherical deconvolution"
)
parser.add_argument(
...
parser.add_argument(
    "masks_path",
    type=str,
    help="path to folder containing brain masks. the mask of x.nii is expected to be named x_mask.nii.gz",
)
parser.add_argument(
    "output_dir", type=str, help="path to folder in which to save the estimated FODs"
)
...
for dwi_nii_directory in extracted_images_path.glob("*/*/*/dwi/"):
    ...
    if response_output_file.exists():
        print(
            f"Skipping response function estimate for {basename} since the following output file exists:\n{response_output_file}"
        )
        continue
    ...
    bvecs[:, 0] = -bvecs[
        :, 0
    ]  # https://github.com/brain-microstructure-exploration-tools/abcd-noddi-tbss-experiments/issues/7
...
for dwi_nii_directory in extracted_images_path.glob("*/*/*/dwi/"):
    ...
    if subject_output_file.exists():
        print(
            f"Skipping {basename} and assuming it was already processed since the following output file exists:\n{subject_output_file}"
        )
        continue
