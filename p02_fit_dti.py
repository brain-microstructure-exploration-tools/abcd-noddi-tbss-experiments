...
parser = argparse.ArgumentParser(
    description="fits diffusion tensors to a diffusion weighted image"
)
parser.add_argument(
...
parser.add_argument(
    "output_dir", type=str, help="path to folder in which to save the DTIs"
)
...
for dwi_nii_directory in extracted_images_path.glob("*/*/*/dwi/"):
    ...
    if subject_output_dir.exists():
        print(
            f"Skipping {basename} and assuming it was already processed since the following output directory exists:\n{subject_output_dir}"
        )
        continue
    ...
    dti_lotri = (
        tenfit.lower_triangular()
    )  # lotri means lower triangular, in dipy order (Dxx, Dxy, Dyy, Dxz, Dyz, Dzz)
