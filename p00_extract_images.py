...
parser.add_argument(
    "table_path",
    type=str,
    help="path to csv table with columns for filename, site_id_l, and mri_info_manufacturer",
)
parser.add_argument(
    "archive_path",
    type=str,
    help="path to folder ABCD images were downloaded (this folder would contain the .tgz files)",
)
parser.add_argument(
    "output_path", type=str, help="path to folder in which to put extracted ABCD images"
)
...
for filename in df.filename:
    ...
    if len(img_path_list) > 1:
        ...
        raise Exception(
            f"Multiple image files were found in {extraction_target}:\n\t{img_path_list_formatted}"
        )
        # (Note that multiple images could happen in general on an ABCD download if someone extracted everything to the same
        # target folder instead of creating one folder for each archive as I did. Because the Phillips scans would be in two parts
        # and those would end up together in the same subriectory.)
    ...
    if len(nii_path_list) > 0:
        ...
        print(
            f"Found that {niigz_path_list[0]} already exists. Skipping recompression."
        )

df_with_basenames = pd.merge(
    df, pd.DataFrame({"filename": filenames, "basename": basenames})
)
